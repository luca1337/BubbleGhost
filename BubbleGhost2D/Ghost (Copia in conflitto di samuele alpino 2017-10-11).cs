...
namespace BehaviourEngine.Test
{
    public class Ghost : GameObject
    {
        ...
        public Ghost( string fileName, int life, Vector2 drawPosition, GameObject target, float range = 120 ) : base( ( int )RenderLayer.Pawn, "Ghost" )
        {
            ...
            AddBehaviour < SwitchAnimation >( new SwitchAnimation( this, target, range,true ) );
            ...
            AddBehaviour < Box2D >( box );

            //Behaviours
            AddBehaviour < Rotator >( new Rotator( this, target, range ) );
            AddBehaviour < Blower >( new Blower( this, target.GetComponent < Blowable >( ), range, 3 ) );

            //Input
            input                             = AddBehaviour < InputMove >( new InputMove( this ) );
            input.Speed                       = 90.0f;

            //Rollback
            rollBack                          = AddBehaviour <RollBack>( new RollBack( this ) );
            rollBack.box                      = this.box;

            //Lifes
            lifes = AddBehaviour < Lifes >( new Lifes( life, target, this ) );
        }
    }
